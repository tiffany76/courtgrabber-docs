...
Your typical workflow might look like this: login to Courtgrabber, add a new reservation request, verify that the request was successful, and then logout. Here are the corresponding API calls for these tasks:
...
You can also cancel pending requests with an API request to [`DELETE /api/reservation-requests/{id}`](#cancel-a-reservation-request).
